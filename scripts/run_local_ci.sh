...
# run-local-ci.sh - Run GitHub CI pipeline locally with caching and detailed logs (macOS)
...
main() {
    ...
    # This script targets macOS only
    if [[ "$OSTYPE" != "darwin"* ]]; then
        print_error "This script is intended for macOS only (detected: $OSTYPE)"
        pause_before_exit
        exit 1
    fi
    ...
    echo -e "${CYAN}║         Pitik Frontend Local CI Runner (macOS)        ║${NC}" | tee -a "$LOG_FILE"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════╝${NC}" | tee -a "$LOG_FILE"
    echo "" | tee -a "$LOG_FILE"
